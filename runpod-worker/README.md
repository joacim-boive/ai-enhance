...
4. Streams the mp4 to the **presigned** `upload_url` the app minted for `users/{userId}/jobs/{jobId}/output.mp4`.
5. Returns `{ object_key, byte_size, etag }` only. No bytes go back through RunPod or Next.js.
...
Set `LUMEN_WORKER_REF` if the files live on another branch.
