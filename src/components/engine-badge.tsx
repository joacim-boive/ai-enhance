...
export function EngineBadge({ health }: Props) {
  if (!health) {
    return (
      <div className="max-w-full rounded-2xl border border-[var(--line)] px-3 py-2 font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--muted)] sm:rounded-full sm:text-[11px] sm:tracking-[0.16em]">
        Checking engines
      </div>
    );
  }
  ...
  return (
    <div className="flex min-w-0 max-w-full flex-wrap items-center gap-x-2 gap-y-1 rounded-2xl border border-[var(--line)] bg-black/30 px-3 py-2 font-mono text-[10px] uppercase tracking-[0.12em] sm:rounded-full sm:text-[11px] sm:tracking-[0.16em]">
      <span className={cpuOk ? "text-[var(--teal)]" : "text-[var(--err)]"}>
        {cpuOk ? "CPU ready" : "CPU missing"}
      </span>
